...
if (($username.Length -lt 1) -or ($password.Length -lt 1)) {
    ...
    $headers.Add("authorization", $response.token_type + " " + $response.access_token)
    
    $products = Invoke-RestMethod -Method Get -Headers $headers -Uri "https://owner-api.teslamotors.com/api/1/products"

    $products.response.Item(0)
}